...
from baseline_model.data_tools import CustomImageDataset, split_train_val, preprocess_resnet50

it = pytest.mark.it
describe = pytest.mark.describe

@describe("Test the class CustomImageDataset and load and prepare data for ResNet50 header")
class TestDataTools:

    # Mock data
    mock_image_label_dict = {
    ...
    @mock.patch("builtins.open", new_callable=mock.mock_open, read_data=mock_image_list_content)
    def test_split_train_val(self, mock_open, mock_isdir):
        mock_isdir.return_value = True
        train_images_dict, val_images_dict, report = split_train_val(self.mock_train_path, prop=0.5, classes=[0, 1])
    ...
    def test_preprocess_resnet50(self, mock_dataloader):
        ...
        dataloader = preprocess_resnet50(self.mock_image_label_dict, batch_size=32, num_workers=4)
